...
%   \caption{EvolveGCN Unit伪代码(GUR+GCN版)}
%   \label{alg:EvovleGCN-Unit}
%   \hspace*{0.04in}{\bf Inputs:}\\
%   前一时刻GCN的参数,亦即前一时刻RNN的隐藏单元$\Theta_{t-1}^{l}$ \\
%   当前时刻GCN的输入特征映射，亦即当前时刻RNN的输入单元$\mathnormal{X}_{t}^{l}$ \\
%   \begin{algorithmic}[1]
%     \State  \\
%     % $\Theta_{t}^{l} = gru(\mathnormal{X}_{t}^{l}, \Theta_{t-1}^{l})$ \\
